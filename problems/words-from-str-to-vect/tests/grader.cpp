...
// std::vector<std::string> has no operator<<, so POTD_ASSERT_EQ (which
...
// POTD_ASSERT_EQ on the individual strings, so a mismatch reports exactly
...
static void assert_vectors_equal(const std::vector<std::string>& actual,
    ...
    POTD_ASSERT_EQ(actual.size(), expected.size());
    for (std::size_t i = 0; i < expected.size(); ++i) {
        POTD_ASSERT_EQ(actual[i], expected[i]);
    }
}

POTD_TEST("\"Hello, World!\" splits into two words with punctuation removed") {
    assert_vectors_equal(WordsToVector("Hello, World!"), {"Hello", "World"});
}

POTD_TEST("\"What is your name?\" splits into four words with the trailing ? removed") {
    assert_vectors_equal(WordsToVector("What is your name?"), {"What", "is", "your", "name"});
}

POTD_TEST("\"Who? What? Where? Why? How?\" removes a ? from every word") {
    assert_vectors_equal(WordsToVector("Who? What? Where? Why? How?"),
                          {"Who", "What", "Where", "Why", "How"});
}

POTD_TEST("a trailing period is removed") {
    assert_vectors_equal(WordsToVector("Yes."), {"Yes"});
}

POTD_TEST("a trailing comma is removed") {
    assert_vectors_equal(WordsToVector("Wait,"), {"Wait"});
}

POTD_TEST("all occurrences of punctuation within a single word are removed, not just the first") {
    assert_vectors_equal(WordsToVector("Really??"), {"Really"});
}

POTD_TEST("a word with no punctuation is returned unchanged") {
...
POTD_TEST("the driver's sample phrase splits and cleans correctly") {
    std::string phrase =
