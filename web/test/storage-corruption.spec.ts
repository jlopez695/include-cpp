...
describe('storage: corrupt localStorage values must not throw', () => {
  ...
  it('getStreak returns 0 when cpp:solve-dates is not valid JSON', () => {
    store['cpp:solve-dates'] = '{not json';
    assert.equal(getStreak(), 0);
  });

  it('getStreak returns 0 when cpp:solve-dates parses to a non-array', () => {
    store['cpp:solve-dates'] = '"hello"';
    assert.equal(getStreak(), 0);
  });

  it('getStreak returns 0 when cpp:solve-dates parses to an array of wrong-typed entries', () => {
    store['cpp:solve-dates'] = JSON.stringify([1, 2, 3]);
    // After filtering non-strings, dates.length === 0 → 0 streak.
    assert.equal(getStreak(), 0);
  });

  it('recordSolveDate recovers from a corrupt cpp:solve-dates and writes a clean value', () => {
    store['cpp:solve-dates'] = 'totally broken';
  ...
  it('recordSolveDate preserves valid prior entries when storage is healthy', () => {
    ...
    store['cpp:solve-dates'] = JSON.stringify([yesterday]);
    recordSolveDate();
    const after = JSON.parse(store['cpp:solve-dates']);
  ...
  it('getBookmarkedIds returns [] when cpp:bookmarks is not valid JSON', () => {
    store['cpp:bookmarks'] = '[POTD0,';
    assert.deepEqual(getBookmarkedIds(), []);
  });

  it('getBookmarkedIds returns [] when cpp:bookmarks parses to a non-array', () => {
    store['cpp:bookmarks'] = JSON.stringify({ POTD0: true });
    assert.deepEqual(getBookmarkedIds(), []);
  });

  it('isBookmarked returns false (does not throw) when storage is corrupt', () => {
    store['cpp:bookmarks'] = 'garbage';
    assert.doesNotThrow(() => assert.equal(isBookmarked('POTD0'), false));
  });

  it('toggleBookmark heals a corrupt cpp:bookmarks and adds the new id', () => {
    store['cpp:bookmarks'] = 'broken';
  ...
  it('loadStatus returns "attempted" when storage holds the literal string', () => {
    store['cpp:status:POTD0'] = 'attempted';
  ...
  it('loadStatus normalizes an arbitrary corrupted value to "unsolved"', () => {
    ...
    store['cpp:status:POTD0'] = 'garbage-value';
  ...
  it('loadStatus normalizes the legacy "unsolved" literal to "unsolved" (no-op)', () => {
    ...
    store['cpp:status:POTD0'] = 'unsolved';
    assert.equal(loadStatus('POTD0'), 'unsolved');
  });
